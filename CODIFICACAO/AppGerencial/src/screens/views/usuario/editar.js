...
class EditarUsuario extends Component {
  ...
  componentDidMount() {
    const usuarioData = this.props.dataUsuario.infoUsuario
    const dataFormatada = moment(usuarioData.datanascimento, 'YYYY-MM-DD').format('DD/MM/YYYY')
    ...
    this.setState({ 
      ...
      usuario: {
        ...
        endereco: {
          ...
          numero: `${usuarioData.endereco.numero}`
        }
      } 
    })
  }
  ...
  onClickCadastrar = async () => {
    ...
    if (msg.trim().length > 0) {
      ...
      // Alterando usuário
      await this.props.dispatch(CarregandoActions.loading())
      await this.props.dispatch(UsuarioActions.updateUser(idusuario, usuario))
      await this.props.dispatch(CarregandoActions.notLoading())
  ...
  render() {
    return (
      <ScrollView>
        ...
        <TopMenu
          onPress={() => this.props.navigation.navigate("InfoUsuario")}
          topPage={'EDITAR USUÁRIO PRINCIPAL'}
        />
        ...
        <View style={Styles.containerButtons}>
          <TouchableOpacity style={[Styles.buttonStyle, { backgroundColor: '#FF0909' }]} onPress={() => this.props.navigation.navigate("InfoUsuario")}>
