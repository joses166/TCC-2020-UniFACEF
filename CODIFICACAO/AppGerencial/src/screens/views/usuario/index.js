...
class Usuario extends Component {
  ...
  onClickCadastrar = async () => {
    ...
    if (msg.trim().length > 0) {
      ...
      const empresa = this.props.dataEmpresa.empresa;

      // Inserindo empresa
      await this.props.dispatch(CarregandoActions.loading())
      await this.props.dispatch(EmpresaActions.createEmpresa(empresa))
      await this.props.dispatch(CarregandoActions.notLoading())
      const empresaInserida = await this.props.dataEmpresa.infoempresa

      if (empresaInserida != null) {
        const usuario = await {
          ...dadosUsuario,
          tipousuario: "A",
          idempresa: empresaInserida.idempresa,
        ...
        if (usuarioInserido && usuarioInserido != null) {
