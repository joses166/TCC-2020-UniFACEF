...
class Empresa extends Component {
  ...
  componentDidMount() {
    if (this.props.dataEmpresa.empresa) {
      if (this.props.dataEmpresa.empresa.length > 5) {
        const dadosempresa = this.props.dataEmpresa.empresa
        this.setState({
          empresa: {
  ...
  onClickAvancar = async () => {
    ...
    if (msg.trim().length > 0) {
      ...
      await this.props.dispatch(CarregandoActions.loading())
      await this.props.dispatch(EmpresaActions.insertEmpresa(dadosEmpresa))
      await this.props.dispatch(CarregandoActions.notLoading())
    ...
    return (
      <ScrollView>
        ...
        <View style={Styles.containerButtons}>
          <TouchableOpacity style={[Styles.buttonStyle, { borderColor: '#FF0909' }]} onPress={() => this.props.navigation.navigate("Inicio")}>
            <Text style={{ color: '#FF0909', fontSize: 16, fontWeight: 'bold' }}>
              VOLTAR
            </Text>
          </TouchableOpacity>
          <TouchableOpacity style={[Styles.buttonStyle, { borderColor: 'rgba(0,203,20, 0.82)' }]} onPress={this.onClickAvancar}>
